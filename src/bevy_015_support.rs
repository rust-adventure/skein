...
use bevy_reflect::{
    prelude::ReflectDefault, NamedField, OpaqueInfo,
    ReflectDeserialize, ReflectSerialize, TypeInfo,
    ...
    match t {
        TypeInfo::Struct(info) => {
            ...
            typed_schema.required = info
                ...
                .collect::<Vec<_>>();
            typed_schema.additional_properties =
                Some(false);
        ...
        TypeInfo::Map(info) => {
            ...
            typed_schema.key_type =
                info.key_ty().ref_type().into();
            typed_schema.value_type =
                info.value_ty().ref_type().into();
        }
        TypeInfo::Tuple(info) => {
            typed_schema.schema_type = SchemaType::Array;
...
fn get_registrered_reflect_types(
    reg: &TypeRegistration,
) -> Vec<String> {
    ...
    let registered_reflect_types: [(TypeId, &str); 5] = [
        {
            (
                TypeId::of::<ReflectComponent>(),
                "Component",
            )
        },
        {
            (
                TypeId::of::<ReflectResource>(),
                "Resource",
            )
        },
        {
            (
                TypeId::of::<ReflectDefault>(),
                "Default",
            )
        },
        {
            (
                TypeId::of::<ReflectSerialize>(),
                "Serialize",
            )
        },
        {
            (
                TypeId::of::<ReflectDeserialize>(),
                "Deserialize",
            )
        },
    ];
...
#[derive(
    Debug, Serialize, Deserialize, Clone, PartialEq, Default,
)]
...
trait SchemaJsonType {
    ...
    fn map_json_type(&self) -> SchemaType {
        match self.get_type_path() {
            ...
            "u8" | "u16" | "u32" | "u64" | "u128"
            | "usize" => SchemaType::Uint,
            "i8" | "i16" | "i32" | "i64" | "i128"
            | "isize" => SchemaType::Int,
            "f32" | "f64" => SchemaType::Float,
            "char" | "str" | "alloc::string::String" => {
                SchemaType::String
            }
            _ => SchemaType::Object,
        }
