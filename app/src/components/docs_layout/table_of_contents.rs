...
pub fn TableOfContents(
    ...
    //   let [currentSection, setCurrentSection] =
    // useState(tableOfContents[0]?.id)

    //   let getHeadings =
    // useCallback((tableOfContents) => {
    ...
    // el.getBoundingClientRect().top - scrollMt
    //         return { id, top }
    //       })
    //       .filter((x) => x !== null)
    //   }, [])
    ...
    //       return false
    //     }
    //     return section.children.findIndex(isActive)
    // > -1   }
